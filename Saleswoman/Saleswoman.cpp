...
/*
 ...
 22:42 test samples
 22:43 fix one bug
 22:58 fix another bug and passed all the samples

 http://kmjp.hatenablog.jp/entry/2017/05/10/0930
...
class Saleswoman {
  ...
  int minMoves(vector<int> pos, vector<int> delta) {
    int bal=0, res=0, N=SZ(pos);
    
    for(int i=N-1; i>=1; --i) pos[i] -= pos[i-1];
