...
/*
 ...
 4:18-4:38 system test passed (489.56 pt)
 
 */
class MulticoreProcessingEasy {
  public:
  int f(int J, int c, int s, int P) {
    int x=(J+c-1)/c;
    int t=(x+s-1)/s;
    return t+P*(c-1);
  }
  int fastestTime(int J, int P, vector<int> S, vector<int> C) {
    int res=numeric_limits<int>::max();
    REP(i,SZ(S)) for(int c=1; c<=C[i]; ++c) {
      res=min(res,f(J,c,S[i],P));
    }
    return res;
  }
};
