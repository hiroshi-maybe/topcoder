...
/*
 ...
 8:38-8:42 system test passed (244.40 pt)
 
 editorial:
  - https://apps.topcoder.com/wiki/display/tc/SRM+621
  - http://torus711.hatenablog.com/entry/20140521/1400680802
   - I didn't know `std::is_sorted()`
 
 */
class TwoWaysSorting {
  public:
  string sortingMethod(vector<string> SS) {
    vector<string> SLC=SS,SLL=SS;
    
    sort(SLC.begin(),SLC.end());
    sort(SLL.begin(),SLL.end(),[&](const string a, const string b) {
      return SZ(a)<SZ(b);
    });
    
    if(SS==SLC&&SS==SLL) return "both";
    if(SS==SLC) return "lexicographically";
    if(SS==SLL) return "lengths";
    return "none";
  }
};
