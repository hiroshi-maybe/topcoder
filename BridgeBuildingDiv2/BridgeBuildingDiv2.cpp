...
/*
 ...
 22:20-22:29 designed
 22:29-22:40 (350.50 pt)
 
 editorials:
  - http://kmjp.hatenablog.jp/entry/2015/06/15/0930
  - 
...
class BridgeBuildingDiv2 {
  ...
  int minDiameter(vector<int> A, vector<int> B, int K) {
    ...
    REP(mask,1<<N) {
      ...
      ZERO(mx);
      REP(i,2*N)REP(j,2*N) if(i!=j) mx[i][j]=Inf;
      REP(i,SZ(A)) mx[i][i+1]=mx[i+1][i]=A[i];
      REP(i,SZ(B)) mx[N+i][N+i+1]=mx[N+i+1][N+i]=B[i];
      
      REP(i,N) if(mask&(1<<i)) mx[i][N+i]=mx[N+i][i]=0;
      
      REP(k,V)REP(i,V)REP(j,V) mx[i][j]=min(mx[i][j],mx[i][k]+mx[k][j]);
      
      int d=0;
      REP(i,V)REP(j,V) d=max(d,mx[i][j]);
      res=min(res,d);
    }
