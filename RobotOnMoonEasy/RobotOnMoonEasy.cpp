...
/*
 ...
 10:25-10:32 system test passed (232.84 pt)
 
 editorials:
  - https://ik-fib.com/2015/03/unrated-srm651-div2/
...
class RobotOnMoonEasy {
  ...
  string isSafeCommand(vector<string> B, string S) {
    int r=-1,c=-1;
    int M=SZ(B),N=SZ(B[0]);
    REP(i,M)REP(j,N) if(B[i][j]=='S') r=i,c=j;
    assert(r>=0&&c>=0);
    FORR(x,S) {
      int r2=r,c2=c;
      if(x=='U') {
      ...
      if(r2<0||r2>=M||c2<0||c2>=N) return "Dead";
      if(B[r2][c2]=='#') continue;
      r=r2,c=c2;
    }
