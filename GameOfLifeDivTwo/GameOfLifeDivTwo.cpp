...
/*
 ...
 8:33-8:40 ACC (232.83 pt)
 
 Editorials:
  - http://apps.topcoder.com/wiki/display/tc/SRM+511
  - http://d.hatena.ne.jp/kusano_prog/20110702/1309632959
  - http://purple-jwl.hatenablog.com/entry/20130322/1363947143
  - https://www.muo.jp/2011/07/topcoder-srm-srm-511-div2-easy.html
 
 */
class GameOfLifeDivTwo {
  public:
  string theSimulation(string res, int T) {
    int N=SZ(res);
    REP(_,T) {
      string X=res;
      REP(i,N) {
        int cnt=0;
        for(int d=-1; d<=1; ++d) cnt+=X[(i+d<0?N+i+d:i+d)%N]=='1';
        res[i]=cnt>=2?'1':'0';
      }
    }
    return res;
