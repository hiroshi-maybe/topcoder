...
/*
 ...
 9:08-9:19 submit (218.13 pt), but 1WA
 22:40-22:42 bug fix and ACC
 
 Editorials:
  - http://topcoder.g.hatena.ne.jp/JAPLJ/20110331
  - http://d.hatena.ne.jp/kusano_prog/20110330/1301499485
  - http://peryaudo.hatenablog.com/entry/20110330/1301502233
  - http://kg86.hatenablog.com/entry/20130517/1368817346
  - http://kyuridenamida.hatenablog.com/entry/2011/03/30/231757
 
 Summary:
  - I missed a case that ratio is not integer BUT arithmetic progression is correct 😞
...
class FoxProgression {
  ...
  int theCount(vector<int> S) {
    if(SZ(S)==1) return -1;
    int dif=S[1]-S[0],rat=S[1]/S[0];
//    if(S[1]%S[0]!=0) return 1;
    dump2(dif,rat);
    int N=SZ(S);
    bool ok1=true,ok2=S[1]%S[0]==0;
    REP(i,N-1) {
      ok1&=S[i+1]==S[i]+dif;
      ok2&=S[i+1]==S[i]*rat;
    }
