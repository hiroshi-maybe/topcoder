...
/**
 ...
 9:11-9:21 passed system test (221.33 pt)
 
 editorial:
  - https://apps.topcoder.com/wiki/display/tc/SRM+667
    - 4 corners (-100,-100),(-100,100),(100,-100),(-100,-100) satisfy C!=A AND C!=B
    - find (C_X,C_Y) s.t. abs(C_X-A_X)>abs(C_X-B_X) AND abs(C_Y-A_Y)>abs(C_Y-B_Y)
 
 8/18/2017
  - add simpler solution of editorial
...
public:
  vector<int> findPoint(int x1, int y1, int x2, int y2) {
    int x3=x1<=x2?100:-100,y3=y1<=y2?100:-100;
    return {x3,y3};
  }
};

class PointDistance_org {
  public:
...
// CUT begin
ifstream data("PointDistance.sample");
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PointDistance (250 Points)" << endl << endl;
    }
