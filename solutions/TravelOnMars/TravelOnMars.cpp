...
/*
 ...
 http://kmjp.hatenablog.jp/entry/2013/06/20/0900
...
class TravelOnMars {
  public:
  int minTimes(vector<int> A, int st, int ds) {
    int N=SZ(A);
    VV<int> G(N);
    REP(i,N) FORE(d,-A[i],A[i]) {
      G[i].push_back(((i+d)%N+N)%N);
    }
    const int Inf=1e8;
    VI D(N,Inf);
    queue<int> Q;
