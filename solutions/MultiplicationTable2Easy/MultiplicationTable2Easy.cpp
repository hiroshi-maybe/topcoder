...
/**
 ...
 17:36-17:46 (222.43 pt)
 
 24:17 read editorials
 - https://apps.topcoder.com/wiki/display/tc/SRM+685
 
 */
class MultiplicationTable2Easy {
  public:
  string ok="Good",ng="Not Good";
  int N;
  VI S;
  string isGoodSet(vector<int> SS, vector<int> T) {
    S=SS;
    N=sqrt(SZ(S));
    int M=SZ(T);
    
    SETI TS(T.begin(),T.end());
    REP(i,M) REP(j,M) {
      int x=T[i],y=T[j];
      if(TS.count(f(x,y))==0) return ng;
    }
