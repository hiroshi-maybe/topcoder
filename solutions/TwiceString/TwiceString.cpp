...
class TwiceString {
  ...
  string getShortest(string S) {
    int L=SZ(S);
    FORE(i,1,L) {
      string s=S.substr(0,i)+S;
//      dump2(S,s.substr(0,L));
      if(S==s.substr(0,L)) return s;
    }
    return "";
  }
};
