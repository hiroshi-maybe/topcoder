...
/*
 ...
 Summary:
  - I missed a pre-condition that `A` does not include `z` 😫
  - Order matters. Due to unsufficient proof, 1WA
...
public:
  ...
  string canTransform(string const &A,
    ...
    // This is not necessary because `A` does not contain `z`
...
bool do_test(string init, string goal, string __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(init, goal, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1510800387;
