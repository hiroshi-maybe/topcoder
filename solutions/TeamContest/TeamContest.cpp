...
class TeamContest {
  ...
  int worstRank(vector<int> strength) {
    int s0=*max_element(strength.begin(),strength.begin()+3)+*(min_element(strength.begin(),strength.begin()+3));
    dump(s0);
    VI A; FOR(i,3,SZ(strength))A.push_back(strength[i]);
    sort(ALL(A));
    dumpAR(A);
    int N=SZ(A);
    int r=N-1;
    if(r<0) return 1;
    int l=upper_bound(ALL(A),s0-A[r])-A.begin();
    int res=0;
    while(l<r-1) {
      ++res;
      dump2(l,r);
      --r,l+=2;
      while(l<r-1&&A[l]+A[r]<=s0) ++l;
//      dump2(l,r);
    }
    return res+1;
  }
