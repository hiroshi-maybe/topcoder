...
/*
 ...
 16:32-16:38 system test passed (239.66 pt)
 
 Editorial:
  - https://apps.topcoder.com/wiki/display/tc/SRM+565
  - http://kmjp.hatenablog.jp/entry/2012/12/22/1030
  - http://ekaing.hatenablog.com/entry/2013/04/20/221716
 
 */
class ValueHistogram {
  public:
  vector<string> build(vector<int> vs) {
    int cnt[10]={};
    int N=SZ(vs);
    REP(i,N) ++cnt[vs[i]];
    
    int H=*max_element(cnt,cnt+10);
    ++H;
    
    vector<string> res;
    for(int i=H; i>0; --i) {
      string s(10,'.');
      REP(x,10) if(cnt[x]>=i) s[x]='X';
      res.push_back(s);
    }
