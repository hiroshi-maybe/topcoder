...
/*
 ...
 10:16-10:25 analysis
 10:26-10:39 implement and submit (333.31 pt). However 1WA
 11:32-11:34 overflow bug fix 😵
 
 Editorials:
  - https://apps.topcoder.com/wiki/display/tc/SRM+516
  - http://techtipshoge.blogspot.com/2011/08/
  - http://d.hatena.ne.jp/kusano_prog/20110830/1314725286
...
class NetworkXOneTimePad {
  ...
  LL num(string s) {
    LL res=0;
    REP(i,N) if(s[i]=='1') res|=1LL<<(N-i-1);
    return res;
  }
  int crack(vector<string> PP, vector<string> CC) {
    N=SZ(PP[0]);
    REP(i,SZ(PP)) P.push_back(num(PP[i]));
    REP(i,SZ(CC)) C.push_back(num(CC[i]));
    
    unordered_set<LL> K;
    unordered_set<LL> S(P.begin(),P.end());
    REP(i,SZ(C)) REP(j,SZ(P)) {
      LL k=C[i]^P[j];
      bool ok=true;
      FORR(n,C) ok&=S.count(n^k)>0;
      if(ok) K.emplace(k);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "NetworkXOneTimePad (500 Points)" << endl << endl;
    }
