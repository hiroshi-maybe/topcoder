...
/*
 ...
 14:40-15:26 AC (112.09 pt)

 https://www.topcoder.com/blog/single-round-match-749-editorials/
 http://kmjp.hatenablog.jp/entry/2019/02/05/0930
...
class FightMonsterDiv1 {
  ...
  __int128 H,A,M,D;
  bool ok(__int128 x) {
    __int128 d=(A+A+M*(x-1))*x/2;
    if(d>=H) return true;
    __int128 k=max((__int128)0,x-D-1);
    __int128 d1=k==0?0:(A+A+M*(k-1))*k/2;
    __int128 dur=min(x-1,D);
    __int128 d2=((A+M*k)+(A+(k+dur-1)*M))*dur/2;
    d2*=5;
    if(d1+d2>=H) return true;
    return false;
  }
  long long fightTime(long long hp, long long attack, int level, long long duration) {
    this->H=hp,this->A=attack,this->M=attack/100*level,this->D=duration;
    __int128 good=1e12,bad=0;
//    cerr<<good<<endl;
    while(good-bad>1) {
      __int128 m=(good+bad)/2;
      (ok(m)?good:bad)=m;
    }
    return (LL)good;
  }
