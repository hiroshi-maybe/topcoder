...
/*
 ...
 13:56-14:00 system test passed (245.91 pt)
...
// CUT begin
ifstream data("EllysSubstringSorter.sample");
...
bool do_test(string S, int L, string __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(S, L, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1508619382;
