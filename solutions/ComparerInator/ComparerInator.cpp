...
/*
 ...
 16:15-16:21 ACC (240.17 pt)
 
 Editorials:
  - https://apps.topcoder.com/wiki/display/tc/SRM+504
  - http://d.hatena.ne.jp/kusano_prog/20110426/1303848152
  - http://kg86.hatenablog.com/entry/20130627/1372343720
  - http://d.hatena.ne.jp/michey1001/touch/20110429/1304096164
 
 */
class ComparerInator {
  public:
  int makeProgram(vector<int> A, vector<int> B, vector<int> X) {
    int N=SZ(A);
    ...
    ok=true;
    REP(i,N) ok&=(A[i]<B[i]?A[i]:B[i])==X[i];
    if(ok) return 7;

    ok=true;
    REP(i,N) ok&=(A[i]<B[i]?B[i]:A[i])==X[i];
    if(ok) return 7;
