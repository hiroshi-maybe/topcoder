...
/*
 ...
 8/3/2019 TCO19 JP region onsite
...
class ChangesInTheLead {
  ...
  int count(int H, int A, int C) {
    ...
    REPE(i,H){
      REPE(j,A)REPE(k,C)REP(l,3) {
        if(i+1<=H) {
          int lead=(i==j)?1:0;
          int ll;
          if((l==0||l==2)&&lead>0) ll=1;
          else ll=l;
          int kk=(l==2)&&lead?k+1:k;
//          if(i==1&&j==1&&k==0&&l==2) dump(ll,kk);
          if(kk<=C) dp[i+1][j][kk][ll]+=dp[i][j][k][l];
        }
        if(j+1<=A) {
          int lead=(i==j)?1:0;
          int ll;
          if((l==0||l==1)&&lead>0) ll=2;
          else ll=l;
          int kk=(l==1)&&lead?k+1:k;
//          if(i==0&&j==0) dump(kk,ll);
          if(kk<=C) dp[i][j+1][kk][ll]+=dp[i][j][k][l];
        }
      }
    }

    return (dp[H][A][C][0]+dp[H][A][C][1]+dp[H][A][C][2]).val;
  }
};
