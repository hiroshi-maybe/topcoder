...
/*
 ...
 8:48-8:52 system test passed (243.91 pt)
 
 editorial:
  - https://apps.topcoder.com/wiki/display/tc/SRM+601
  - http://torus711.hatenablog.com/entry/20131222/p3
 
 */
class WinterAndMandarins {
  public:
  int getNumber(vector<int> X, int K) {
    sort(X.begin(),X.end());
    int res=1e9+5;
    REP(i,SZ(X)) if(i+K-1<SZ(X)) res=min(res,X[i+K-1]-X[i]);
    return res;
  }
};
