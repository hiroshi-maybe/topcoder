...
/**
 ...
 Add the same greedy solution as solved div1 easy in the contest
 
 16:40 try DP solution too
 17:05 accepted
 */

// O(N^4) time, DP solution
class MaximumRangeDiv2 {
public:
  int findMax(string s) {
    unordered_map<int, unordered_map<int, SETI>> dp;
    dp[0][0].insert(0);
    
    for(auto &c : s) {
      unordered_map<int, unordered_map<int, SETI>> temp;
      int d = c=='+' ? 1 : -1;
      for(auto &minkvp : dp) {
        int minv = minkvp.first;
        for(auto &maxkvp : minkvp.second) {
          int maxv = maxkvp.first;
          for(auto &val : maxkvp.second) {
            temp[minv][maxv].insert(val);
            temp[min(val+d,minv)][max(val+d,maxv)].insert(val+d);
          }
        }
      }
...
// same as div1 MaximumRange
class MaximumRangeDiv2_greedy {
  public:
  int findMax(string s) {
    int pn=0, nn=0;
    for(auto &c : s) {
