...
/*
 ...
 8:45-8:49 ACC (243.61 pt)
 
 Editorials:
  - https://apps.topcoder.com/wiki/display/tc/SRM+515
  - http://d.hatena.ne.jp/kusano_prog/20110821/1313934821
  - http://kg86.hatenablog.com/entry/20130708/1373302781
  - http://d.hatena.ne.jp/nise_nabe/20110822/1313968347
...
class FortunateNumbers {
  ...
  bool ok(int x) {
    ...
    bool ok=true;
    REP(i,SZ(s)) ok&=s[i]=='5'||s[i]=='8';
    return ok;
  }
  int getFortunate(vector<int> A, vector<int> B, vector<int> C) {
    SETI S;
    REP(i,SZ(A))REP(j,SZ(B))REP(k,SZ(C)) {
      int x=A[i]+B[j]+C[k];
      if(ok(x)) S.emplace(x);
    }
    return SZ(S);
