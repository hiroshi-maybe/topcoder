...
/*
 ...
 10/10/2019 SRM 768 div1
 
 8:00-8:42 AC
 
 It took tons of time that grade is at most 10 condition is missing..
 
 https://docs.google.com/document/d/1XbeqZMHsNjv2J3h8x0PsJnBn4IV8llJx9hT39iQkOcY/edit
 
 */
const int Inf=1e9;
int f(int sum, int B, VI &D) {
    int ds=0,N=SZ(D);
    VI G(N,0);
    FOR(i,N/2,N) {
        ds+=D[i]*B;
    ...
    int rem=max(0,sum-accumulate(ALL(G),0));
    for(int i=N-1; i>=0; --i) if(G[i]<10) {
        int dif=min(10-G[i],rem);
        ds+=dif*D[i],rem-=dif;
    }
    if(rem>0) return Inf;
    return ds;
...
class MeanMedian {
  public:
  int minEffort(int A, int B, vector<int> D) {
    sort(ALL(D)),reverse(ALL(D));
//    dumpc(D);
    int res=Inf,N=SZ(D);
    for(int sum=A*N; sum<=N*10; ++sum) {
        SMIN(res,f(sum,B,D));
    }
    return res;
  }
