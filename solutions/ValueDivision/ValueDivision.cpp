...
/*
 ...
 4/24/2020 SRM 784 div1 easy
 
 8:00-8:25 AC
 
 https://kmjp.hatenablog.jp/entry/2020/04/25/0900
...
class ValueDivision {
  ...
  vector<int> f(VI &A) {
    ...
    VI as;
    FORR(kvp,cnt) {
      as.push_back(kvp.first);
      FORR(i,kvp.second) {
        xs[SZ(as)-1].push_back(i);
      }
    }
    ...
    for(int i=SZ(as)-1; i>=0; --i) {
      int a=as[i];
      FORR(j,xs[i]) Q.emplace(j);
      int apre=i==0?1:as[i-1];
      while(a>apre&&SZ(Q)) {
        int j=Q.top(); Q.pop();
        res[j]=a;
        --a;
...
/*
    while(true) {
      int N=genRandNum(1,100);
      VI x=genRandSeq(N,1,20);
      dumpc(x);
      f(x);
    }*/
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "ValueDivision (250 Points)" << endl << endl;
    }
