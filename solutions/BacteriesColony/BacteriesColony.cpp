...
/*
 ...
 15:37-15:47 system test passed (219.76 pt)
 
 editorial:
  - https://apps.topcoder.com/wiki/display/tc/SRM+645
 
 Result convergences to monotonically increasing or decreasing.
 Thus we can run loop at most 100 times.
 
 */
class BacteriesColony {
  public:
  vector<int> performTheExperiment(vector<int> S) {
    int N=SZ(S);
    
    REP(_,105) {
      VI T=S;
      FOR(i,1,N-1) {
        if(S[i-1]>S[i]&&S[i]<S[i+1]) ++T[i];
        else if(S[i-1]<S[i]&&S[i]>S[i+1]) --T[i];
      }
      swap(S,T);
    }
